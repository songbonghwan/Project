...
# 데이터 불러오기
feat_ment = pd.read_csv('csv/정신질환ment.csv')
...
# "정신질환 사망건수"에 해당하는 열을 확인하여 선택 (예: 'Death_Count'라는 열이 있다고 가정)
# feat_ment 데이터에 'AREA'와 'Death_Count'라는 열이 있다고 가정하고, 이를 활용해 예측
cities = feat_ment['AREA'].unique()
...
for city in city_order:  # 지정된 도시 순서대로 반복
    if city in cities:  # 해당 도시가 데이터에 있을 경우에만 진행
        # 각 도시의 데이터 선택
        city_data = feat_ment[feat_ment['AREA'] == city].drop('AREA', axis=1).T
...
# CSV 파일로 저장
forecast_pivot.to_csv('예측된_정신질환_사망건수_2024_2075.csv')
...
plt.title('2024년부터 2075년까지의 도시별 정신질환 사망건수 예측')
plt.xlabel('Year')
