...
# 데이터 불러오기
fertile_women = pd.read_csv('csv/가임 여성수_full_1.csv')
...
cities = fertile_women['AREA'].unique()
...
for city in city_order:  # 지정된 도시 순서대로 반복
    if city in cities:  # 해당 도시가 데이터에 있을 경우에만 진행
        # 각 도시의 데이터 선택
        city_data = fertile_women[fertile_women['AREA'] == city].drop('AREA', axis=1).T
        ...
        city_data['Population'] = city_data['Population'].replace({',': ''}, regex=True).astype(float)

        # NaN 값 처리 (NaN을 이전 값으로 대체)
        city_data['Population'] = city_data['Population'].fillna(method='ffill')
...
# CSV 파일로 저장
forecast_pivot.to_csv('예측된_가임_여성수_2024_2075.csv')
...
plt.title('2024년부터 2075년까지의 도시별 가임 여성수 예측')
plt.xlabel('Year')
