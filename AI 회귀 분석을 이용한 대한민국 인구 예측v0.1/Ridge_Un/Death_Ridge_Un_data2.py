...
for region in regions:
    ...
    # 4. Ridge 모델 훈련
    ridge_model = Ridge(alpha=1.0, max_iter=10000, random_state=42)
    ...
    print(f"Train score with Ridge for {region}: {train_score}")
    print(f"Test score with Ridge for {region}: {test_score}")
    ...
    # 예측값 계산
    future_predictions = ridge_model.predict(X_future_scaled)
